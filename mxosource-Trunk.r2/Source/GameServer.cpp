...
void GameServer::Loop(void)
{
	...
	TIMING_LOG ( "PruneDeadClients Start" ) ;
	m_mainSocket->PruneDeadClients();
	TIMING_LOG ( "PruneDeadClients End" ) ;
	TIMING_LOG ( "CheckAndResend Start" ) ;
	m_mainSocket->CheckAndResend();
	TIMING_LOG ( "CheckAndResend End" ) ;

	TIMING_LOG ( "UpdateClients Start" ) ;
	m_mainSocket->UpdateClients();
	TIMING_LOG ( "UpdateClients End" ) ;
	
		// Loop processes about once every 4ms, this if processes about once every 4 seconds.
	if ( itteration % 1000 == 0 )
	{
		TIMING_LOG ( "ProcessWorld Start" ) ;
		m_mainSocket->ProcessWorld();
		TIMING_LOG ( "ProcessWorld End" ) ;
	}
	
	TIMING_LOG ( "udpHandler Start" ) ;
	m_udpHandler.Select(0,4000); //4ms
	TIMING_LOG ( "udpHandler End" ) ;
