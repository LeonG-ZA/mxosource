...
#include "ControlRunnable.h"
#include "MarginRunnable.h"
#include "MergedRequestRPC.h"
#include "MergedResponseRPC.h"
#include "ModelRunnable.h"
...
bool Master::Run()
{
	...
	MergedRequestRPC* requestRPC = new MergedRequestRPC () ;
	MergedResponseRPC* responseRPC = new MergedResponseRPC () ;

	//start server threads
	AuthRunnable *authRun = new AuthRunnable();
	ThreadPool.ExecuteTask(authRun);
	ModelRunnable *modelRun = new ModelRunnable();
	ThreadPool.ExecuteTask ( modelRun ) ;
	MarginRunnable *marginRun = new MarginRunnable( modelRun->GetWorldModel() );
	ThreadPool.ExecuteTask(marginRun);
	GameRunnable *gameRun = new GameRunnable( modelRun->GetWorldModel() , ( ReceiveRequestRPC* )requestRPC , ( HandleResponseRPC* )responseRPC );
	ThreadPool.ExecuteTask(gameRun);
	ControlRunnable *controlRun = new ControlRunnable ( ( HandleRequestRPC* )requestRPC , ( ProduceResponseRPC* )responseRPC , modelRun->GetWorldModel() );
	ThreadPool.ExecuteTask(controlRun);
