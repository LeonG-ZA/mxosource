...
class ReceiveRequestRPC : public RequestRPC
{
	...
	public :
		...
		ReceiveRequestRPC ( queue < RequestRPCFunction* >* NewRequestList ) ;
			...
			//
		void Jump ( PlayerCharacterModel* JumpCharacter , LocationVector JumpTarget ) ;
			...
			//
		void Chat ( PlayerCharacterModel* SourceCharacter , std::string ChatMessage ) ;

			// Unsigned parameters should probably become enums when they are fully identified...
		...
		void TacticUpdate ( PlayerCharacterModel* TacticCharacter , unsigned TacticIndex ) ;
