...
public:
	...
	~ObjectUpdateMsg();
	virtual void setReceiver(CharacterModel* MessageTarget);
protected:
...
{
public:
	PlayerSpawnMsg(CharacterModel* MessageCharacter);
	~PlayerSpawnMsg();
	const ByteBuffer& toBuf();
};
...
class WorldChatMsg : public MsgBaseClass
{
public:
	WorldChatMsg ( CharacterModel* MessageCharacter , std::string NewMessage ) ;
	~WorldChatMsg ( void ) ;
