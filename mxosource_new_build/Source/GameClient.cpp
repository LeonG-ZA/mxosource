...
void GameClient::HandlePacket( const char *pData, size_t nLength )
{
	...
	if (m_encryptionInitialized == false && pData[0] == 0 && nLength == 43)
	{
		...
		try
		{
			...
			sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->SetGOID ( m_playerGoId ) ;
		}
...
{
	QueueCommand(make_shared<LoadWorldCmd>((LoadWorldCmd::mxoLocation)(sGame.GetWorldModel()->FindCharacter(m_characterUID)->GetDistrict()),"Massive"));
	QueueCommand(make_shared<SetExperienceCmd>(sGame.GetWorldModel()->FindCharacter(m_characterUID)->GetExperience()));
	QueueCommand(make_shared<SetInformationCmd>(sGame.GetWorldModel()->FindCharacter(m_characterUID)->GetCash()));
			...
			if (srcCmd.remaining())
			{
				...
				switch ( shortCommand )
				{
					...
					case 0x2810 :
					{
						...
						sGame.GetRPCBuffer ()->Chat ( sGame.GetWorldModel ()->FindCharacter ( m_characterUID ) , theMessage ) ;
					...
					case 0x80c2 :
					{
						...
						CharacterModel* jumpCharacter = sGame.GetWorldModel ()->FindCharacter ( m_characterUID ) ;
...
void GameClient::SpawnSelf()
{
	...
	shared_ptr<PlayerSpawnMsg> dMsg = make_shared<PlayerSpawnMsg>(sGame.GetWorldModel()->FindCharacter(m_characterUID));
...
void GameClient::PopulateWorld()
{
	...
	CharacterModel* theOtherCharacter = sGame.GetWorldModel ()->GetFirstCharacter () ;
	while ( theOtherCharacter != NULL )
	{
		...
		theOtherCharacter = sGame.GetWorldModel ()->GetNextCharacter () ;
	}
...
/*	testCount++;
	...
	if (goIdToUpdate != sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->GetGOID ())
	{
		WARNING_LOG(format("Client %1% Player %2%:%3% trying to update someone else's object view %4%") % this->Address() % sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->GetHandle () % sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->GetGOID () % goIdToUpdate);
		return;
	}
	...
	if (shouldBeOne != 1)
	{
		WARNING_LOG(format("Client %1% Player %2%:%3% 03 doesn't have number 1 after viewId, packet: %4%") % this->Address() % sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->GetHandle () % sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->GetGOID () % Bin2Hex(srcData));
		return;
	}
	...
	case 0x04:
		{
			...
			sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->SetHeading ( theRotByte ) ;
			validUpdate=true;
			break;
		}
	...
	case 0x08:
		{
			...
			srcData >> tempX;
			x = tempX ;
			sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->SetPositionX ( x ) ;
			srcData >> tempY;
			y = tempY ;
			sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->SetPositionY ( y ) ;
			srcData >> tempZ;
			z = tempZ ;
			sGame.GetWorldModel ()->FindCharacter ( m_characterUID )->SetPositionZ ( z ) ;
	...
	case 0x02:
		...
		sGame.AnnounceStateUpdate(this,make_shared<StateUpdateMsg>(sGame.GetWorldModel ()->FindCharacter ( m_characterUID ),theStateData),true);
}
