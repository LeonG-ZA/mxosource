...
public:		
	...
	void QueueState(msgBaseClassPtr theData,bool immediateOnly=false,packetAckFunc callFunc=0)
	{
		...
		if (amIObjectUpdate != NULL)
			amIObjectUpdate->setReceiver(sGame.GetWorldModel()->FindCharacter(m_characterUID));
	...
	void InitializeWorld();
		...
		//
		//	Processes 04 blocks to determine what request a cliet is sending
		//	and calls the corresponding function to handle the request.
		//
...
private:
	...
	typedef void (GameClient::*RPCHandler)( ByteBuffer &srcCmd );
	...
	unsigned m_characterUID;
	unsigned m_charWorldId;
