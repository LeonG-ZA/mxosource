...
bool GameServer::Start( WorldModel* NewWorldModel , ReceiveRequestRPC* NewRequestRPC , HandleResponseRPC* NewResponseRPC )
{
	MyWorldModel = NewWorldModel ;
	MyRequestRPC = NewRequestRPC ;
	MyResponseRPC = NewResponseRPC ;
	
	MyResponseHandler = new ResponseHandler () ;
	
	NewResponseRPC->SetResponseHandler ( MyResponseHandler ) ;
...
void GameServer::Loop(void)
{
	...
	while ( MyResponseRPC->ProcessResponse () == HandleResponseRPC::SUCCESS ) ;
