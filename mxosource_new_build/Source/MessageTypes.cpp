...
ObjectUpdateMsg::ObjectUpdateMsg(CharacterModel* MessageCharacter):TheCharacter(MessageCharacter),TargetCharacter(NULL)
{
}
...
void ObjectUpdateMsg::setReceiver( CharacterModel* MessageTarget )
{
	TargetCharacter = MessageTarget;
}
...
PlayerSpawnMsg::PlayerSpawnMsg(CharacterModel* MessageCharacter) : ObjectUpdateMsg(MessageCharacter)
{
...
const ByteBuffer& PlayerSpawnMsg::toBuf()
{
	...
	ERROR_LOG(format("Player %1%:%2% spawn packet serializing for client %3% with viewID %4%") % TheCharacter->GetHandle() % TheCharacter % TheCharacter->GetCharacterID () % goId);
...
WorldChatMsg::WorldChatMsg ( CharacterModel* MessageCharacter , std::string NewMessage )
{
...
const ByteBuffer& WorldChatMsg::toBuf ()
{
	byte sampleSpawnPacket[36] =
	{
	...
	uint16* headerLengthPTR = ( uint16* )&sampleSpawnPacket[11] ;
	*headerLengthPTR = 36 + SourceCharacter->GetHandle ().length () + 3 ;

	m_buf.clear () ;
	m_buf.append ( sampleSpawnPacket , sizeof ( sampleSpawnPacket ) ) ;

		// Add handle of sending character.
	m_buf.writeString ( SourceCharacter->GetHandle () ) ;
