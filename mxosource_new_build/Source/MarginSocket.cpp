	...
	case MS_LoadCharacterRequest:
		{
			...
			//scope for db ptr
			{
				...
				myModel->AddCharacter ( m_userId , charId ) ;

				CharacterModel* myCharacterModel = myModel->FindCharacter ( charId ) ;

				m_charName = myCharacterModel->GetHandle () ;
				m_firstName = myCharacterModel->GetFirstName () ;
				m_lastName = myCharacterModel->GetLastName () ;

				m_background = myCharacterModel->GetBackground () ;
			}
